...
export default function Dashboard({ isLoggedIn, setIsLoggedIn, language }) {
  ...
  // 1. Initialize state from sessionStorage to persist across page refreshes
  const [currentView, setCurrentView] = useState(() => {
    return sessionStorage.getItem('notesroom_currentView') || 'semesters';
  });
  
  const [selectedSemester, setSelectedSemester] = useState(() => {
    const saved = sessionStorage.getItem('notesroom_selectedSemester');
    return saved ? JSON.parse(saved) : null;
  });
  
  const [selectedSubject, setSelectedSubject] = useState(() => {
    const saved = sessionStorage.getItem('notesroom_selectedSubject');
    return saved ? JSON.parse(saved) : null;
  });
  ...
  // 2. Update sessionStorage whenever the view changes
  useEffect(() => {
    sessionStorage.setItem('notesroom_currentView', currentView);
    
    if (selectedSemester) {
      sessionStorage.setItem('notesroom_selectedSemester', JSON.stringify(selectedSemester));
    } else {
      sessionStorage.removeItem('notesroom_selectedSemester');
    }

    if (selectedSubject) {
      sessionStorage.setItem('notesroom_selectedSubject', JSON.stringify(selectedSubject));
    } else {
      sessionStorage.removeItem('notesroom_selectedSubject');
    }
  ...
  const handleLogout = () => {
    ...
    sessionStorage.clear(); // Clear saved views on logout
    setIsLoggedIn(false);
  };
  ...
  // FAQS Array (kept identical to your code)
  const faqs = [
    {
          ...
          <AnimatePresence mode="wait">
            ...
            {currentView === 'documents' && selectedSubject && (
              <DocumentsView 
                ...
                t={t}
                handleProtectedAction={handleProtectedAction}
                setActiveChatDoc={setActiveChatDoc}
                setActivePdfDoc={setActivePdfDoc}
